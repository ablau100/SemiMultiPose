...
weight_p = 'training_dir/25/three_batch/combined_res'+str(alpha)+'/'+version + '/fcos_kps_ms_training_R_50_FPN_1x_bee/model_0007500.pth'
original = torch.load(weight_p)
new_m = {"model": original["model"]}
torch.save(new_m, 'training_dir/250/tb_'+ alpha + version+ '_model_0500.pth')
...
cfg.merge_from_list(['DATALOADER.NUM_WORKERS', 2, \
                     'DATATYPE', 'bee', \
        'OUTPUT_DIR', 'training_dir/' + number_data + '/three_batch/' + loss_type + '_res_f5_'+ alpha + '/' + version +'/fcos_kps_ms_training_R_50_FPN_1x_bee', \
        ...
        'MODEL.WEIGHT', 'training_dir/250/tb_'+ alpha + version+ '_model_0500.pth', \
