...
def train(cfg, local_rank, distributed, loss_type, alpha):
#     assert is_pytorch_1_1_0_or_later()
    ...
    #weight_p = 'training_dir/250/standard_track/1/fcos_kps_ms_training_R_50_FPN_1x_bee/model_0005000.pth'
    #original = torch.load(weight_p)
    #new_m = {"model": original["model"]}
    #torch.save(new_m, 'training_dir/250/standard_track_f5/model_0005000.pth')
...
number_data = "250"#str(sys.argv[2])
...
#original = torch.load(weight_p)
#new_m = {"model": original["model"]}
#torch.save(new_m, 'training_dir/250/standard_track_f25/model_0005000.pth')
...
cfg.merge_from_list(['DATALOADER.NUM_WORKERS', 2, \
                     'DATATYPE', 'bee', \
        'OUTPUT_DIR', 'training_dir/' + number_data + '/' + loss_type + '_fs1_f5/' +version + '/'+alpha +'/fcos_kps_ms_training_R_50_FPN_1x_bee', \
        ...
        'MODEL.WEIGHT', 'training_dir/250/standard_track_f5/model_0005000.pth', \
...
#original = torch.load(weight_p)
#new_m = {"model": original["model"]}
#torch.save(new_m, 'training_dir/250/standard_track_copy_f5/model_0005000.pth')

model = train(cfg, local_rank, distributed, loss_type, alpha)
