...
def build_dataset(dataset_list, transforms, dataset_catalog, is_train=True, unlabeled=False):
    """
    Arguments:
    ...
    for dataset_name in dataset_list:
        ...
        if data["factory"] == "COCODataset":
            args["remove_images_without_annotations"] = is_train and not unlabeled
...
def make_data_loader(cfg, is_train=True,
                     ...
                     start_iter=0, just_dataset=False,
                     use_unlabeled=False,
    ...
    if is_train:
        dataset_list_labeled = cfg.DATASETS.TRAIN
        if use_unlabeled:
            dataset_list_unlabeled = cfg.DATASETS.UNLABELED
            
        transforms = build_transforms(cfg, is_train)
        datasets_labeled = build_dataset(dataset_list_labeled, transforms, DatasetCatalog, is_train)
        if use_unlabeled:
            datasets_unlabeled = build_dataset(dataset_list_unlabeled, transforms, DatasetCatalog, is_train, unlabeled=True)
        else:
            datasets_unlabeled = build_dataset(dataset_list_labeled, transforms, DatasetCatalog, is_train)
            
    if just_dataset == True:
        DatasetCatalog = paths_catalog.DatasetCatalog
        dataset_list = cfg.DATASETS.TEST
        transforms = build_transforms(cfg,is_train=False,just_dataset=True)
    ...
    for (dataset_l, dataset_ul) in zip(datasets_labeled, datasets_unlabeled):
        ...
        images_per_gpu_l = labeled_img_per_gpu
        ...
        if use_unlabeled: 
            images_per_gpu_ul = unlabeled_img_per_gpu
