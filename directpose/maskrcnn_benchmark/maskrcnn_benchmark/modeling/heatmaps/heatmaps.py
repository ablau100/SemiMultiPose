...
class Heatmaps(nn.Module):
    ...
    def prepare_kps_targets(self, targets, stride, heatmap_hw, ground_truth):
        ...
        for im_i in range(len(targets)):
            ...
            # check that ground truth image has keypoints, if not add empty heatmap/scores
            if targets_per_im.has_field("keypoints"):
                keypoints = targets_per_im.get_field("keypoints").keypoints
            else:
                targets_per_im.add_field("keypoints", 
                                         Keypoints(torch.empty((0,self.num_kps,3),
