...
class FCOSLossComputation(object):
    ...
    def get_sample_region(self, gt, strides, num_points_per, gt_xs, gt_ys, radius=1.0):
        ...
        for level, n_p in enumerate(num_points_per):
            ...
            center_gt[beg:end, :, 0] = xmin
            center_gt[beg:end, :, 1] = ymin
            center_gt[beg:end, :, 2] = xmax
            center_gt[beg:end, :, 3] = ymax
            beg = end
    ...
    def compute_targets_for_locations(
        ...
        for im_i in range(len(targets)):
            ...
            kp_centroid = keypoints[:,1,:2]
            kp_nonzero = torch.nonzero(torch.prod(kp_centroid>0,1))[:,0]
            if kp_nonzero.size()[0]==0:
                continue
        
            bboxes_centroid_keypoints = torch.cat(2*[kp_centroid[kp_nonzero,:]],1)
            area_keypoints = area[kp_nonzero]
            bboxes_keypoints = bboxes[kp_nonzero,:]

            l = xs[:, None] - bboxes_keypoints[:, 0][None]
            t = ys[:, None] - bboxes_keypoints[:, 1][None]
            r = bboxes_keypoints[:, 2][None] - xs[:, None]
            b = bboxes_keypoints[:, 3][None] - ys[:, None]
            reg_targets_per_im = torch.stack([l, t, r, b], dim=2)

            is_in_boxes = self.get_sample_region(
                bboxes_centroid_keypoints,
                self.fpn_strides,
            ...
            locations_to_gt_area = area_keypoints[None].repeat(len(locations), 1)
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, kps_pred, targets):
        ...
        if (len(labels)==0):
            zero_const = torch.tensor(0.0, dtype=torch.float32, device=torch.device('cuda:0'))
            return zero_const, zero_const, zero_const
        ...
        print("kps_targets_flatten", kps_targets_flatten.size)
        pdbb
        if pos_inds.numel() > 0:
