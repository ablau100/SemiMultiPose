...
#             ]
    ...
    def forward(self, x):
        ...
        for l, feature in enumerate(x):
            ...
            #print("SHAPE THING", kps_pred.shape)
            #pdb.set_trace()
            if self.training:
...
class FCOSModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None, heatmaps_results=None, heatmaps=None, stride=None, labeled=True):
        ...
        """
        #print("HM in forward")
        #print(heatmaps_results)
       # print("TARGETS: ", targets)
        #print("feats: ", len(features))
        #print("feats: ", len(features[0]))
        #print("feats: ", len(features[0][0]))
        #print(torch.Tensor(features).dtype)
        #for f in features[0]:
         #   print(f)
       ...
       # print("t f: ", type(features))
        #print("dim f[0]: ", features[0].size())
        #features_loss = ()
        #features_loss = features.clone()
        #use_target = []
        ...
        #for i in range(0,len(features)):
         #   tup =  features[i][use_target].detach()
          #  features_loss = features_loss + (tup,)
            
        #print("feats: ", len(features_loss)) 
        #print("t f2: ", type(features_loss))
        #print("dim f2[0]: ", features_loss[1].size())
        #box_cls_loss, box_regression_loss, centerness_loss, kps_pred_loss = self.head(features)#_loss)
        #locations_loss = self.compute_locations(features)#_loss)
        
        #for (f, fl) in zip(features, features_loss):
         #   if torch.all(torch.eq(f, fl)):
          #      print("feature loss good")
           # else:
            #    print("feature loss bad")
    ...
    def _forward_test(
        ...
        # INF = 10000000.
        # for im_i in range(len(boxes)):
        #     boxes_per_im = boxes[im_i]
        #     keypoints_per_im = boxes_per_im.get_field("keypoints").keypoints
        #     bboxes_per_im = boxes_per_im.bbox
        #     heatmaps_per_im = heatmaps[im_i]
        #     for type_i in range(len(heatmaps_per_im)):
        #         heatmaps_per_type = heatmaps_per_im[type_i]
        #         if len(heatmaps_per_type) == 0:
        #             continue
        #         in_boxes = (bboxes_per_im[:, 0][:, None] <= heatmaps_per_type[:, 0][None]) & \
        #                    (bboxes_per_im[:, 2][:, None] >= heatmaps_per_type[:, 0][None]) & \
        #                    (bboxes_per_im[:, 1][:, None] <= heatmaps_per_type[:, 1][None]) & \
        #                    (bboxes_per_im[:, 3][:, None] >= heatmaps_per_type[:, 1][None])
        #         keypoints_per_type = keypoints_per_im[:, type_i, :2]
        #         distances = ((keypoints_per_type[:, None] - heatmaps_per_type[None, :]) ** 2).sum(dim=-1)
        #         distances[~in_boxes] = INF
        #         min_dist, min_inds = distances.min(dim=1)
        #         keypoints_per_type = heatmaps_per_type[min_inds]
        #         keypoints_per_im[:, type_i, :2][min_dist != INF] = keypoints_per_type[min_dist != INF]
