...
class DeformConvExtraStride(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        super(DeformConvExtraStride, self).__init__()
        assert in_channels % groups == 0, \
            'in_channels {} cannot be divisible by groups {}'.format(
                in_channels, groups)
        assert out_channels % groups == 0, \
    ...
    def forward(self, input, offset, stride):
        stride = _pair(stride)
        
        y = deform_conv(input, offset, self.weight, stride,
                           self.padding, self.dilation, self.groups,
                           self.deformable_groups)
        if self.with_bias:
            assert len(y.size()) == 4
